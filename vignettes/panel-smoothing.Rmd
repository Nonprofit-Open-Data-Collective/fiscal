...
straight into `compute_all_panel()` or any downstream step.
...
`{2, 4}` with re-normalized weights `{0.667, 0.333}` (i.e. `0.50 / 0.75` and
`0.25 / 0.75`), giving `2.667`.
...
# then score the cleaned, smoothed panel year by year
panel_ratios <- compute_all_panel(panel_smoothed)
```
