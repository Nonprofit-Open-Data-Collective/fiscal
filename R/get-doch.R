...
#' data( dat10k )
#'
#' d <- get_doch( df = dat10k )
#' head( d[ , c( "doch", "doch_w", "doch_z", "doch_p" ) ] )
#'
#' @export
get_doch <- function( df,
                      cash                = "F9_10_ASSET_CASH_EOY",
                      savings             = "F9_10_ASSET_SAVING_EOY",
                      pledges_receivable  = "F9_10_ASSET_PLEDGE_NET_EOY",
                      accounts_receivable = "F9_10_ASSET_ACC_NET_EOY",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( DOCH$doch, na.rm = TRUE ), main = "DOCH (raw)" )
    plot( density( DOCH$doch_w, na.rm = TRUE ), main = "DOCH Winsorized" )
    plot( density( DOCH$doch_z, na.rm = TRUE ), main = "DOCH Standardized (Z)" )
    plot( density( DOCH$doch_p, na.rm = TRUE ), main = "DOCH Percentile" )
  }

  return( cbind( df, DOCH ) )
}
