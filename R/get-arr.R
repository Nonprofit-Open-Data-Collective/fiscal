...
#'
#' d <- get_arr( df = dat10k )
#' head( d[ , c( "arr", "arr_w", "arr_z", "arr_p" ) ] )
#'
#' @export
get_arr <- function( df,
                     total_assets  = "F9_10_ASSET_TOT_EOY",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( ARR$arr, na.rm = TRUE ), main = "ARR (raw)" )
    plot( density( ARR$arr_w, na.rm = TRUE ), main = "ARR Winsorized" )
    plot( density( ARR$arr_z, na.rm = TRUE ), main = "ARR Standardized (Z)" )
    plot( density( ARR$arr_p, na.rm = TRUE ), main = "ARR Percentile" )
  }
