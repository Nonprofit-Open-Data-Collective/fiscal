...
#' data( dat10k )
#'
#' d <- get_eidr( df = dat10k )
#' head( d[ , c( "eidr", "eidr_w", "eidr_z", "eidr_p" ) ] )
#'
#' @export
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( EIDR$eidr, na.rm = TRUE ), main = "EIDR (raw)" )
    plot( density( EIDR$eidr_w, na.rm = TRUE ), main = "EIDR Winsorized" )
    plot( density( EIDR$eidr_z, na.rm = TRUE ), main = "EIDR Standardized (Z)" )
    plot( density( EIDR$eidr_p, na.rm = TRUE ), main = "EIDR Percentile" )
  }

  return( cbind( df, EIDR ) )
}
