...
#'
#' d <- get_orr( df = dat10k )
#' head( d[ , c( "orr", "orr_w", "orr_z", "orr_p" ) ] )
#'
#' @export
get_orr <- function( df,
                     unrestricted_net_assets = "F9_10_NAFB_UNRESTRICT_EOY",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( ORR$orr, na.rm = TRUE ), main = "ORR (raw)" )
    plot( density( ORR$orr_w, na.rm = TRUE ), main = "ORR Winsorized" )
    plot( density( ORR$orr_z, na.rm = TRUE ), main = "ORR Standardized (Z)" )
    plot( density( ORR$orr_p, na.rm = TRUE ), main = "ORR Percentile" )
  }
