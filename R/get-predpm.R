...
#' data( dat10k )
#'
#' d <- get_predpm( df = dat10k )
#' head( d[ , c( "predpm", "predpm_w", "predpm_z", "predpm_p" ) ] )
#'
#' @export
                     ...
                     summarize = FALSE )
{
  ...
  PREDPM <- data.frame( predpm   = v$raw,
                        predpm_w = v$winsorized,
                        predpm_z = v$z,
                        predpm_p = v$pctile )
  ...
  if ( summarize ) {
    ...
    plot( density( PREDPM$predpm, na.rm = TRUE ), main = "PREDPM (raw)" )
    plot( density( PREDPM$predpm_w, na.rm = TRUE ), main = "PREDPM Winsorized" )
    plot( density( PREDPM$predpm_z, na.rm = TRUE ), main = "PREDPM Standardized (Z)" )
    plot( density( PREDPM$predpm_p, na.rm = TRUE ), main = "PREDPM Percentile" )
  }

  return( cbind( df, PREDPM ) )
}
