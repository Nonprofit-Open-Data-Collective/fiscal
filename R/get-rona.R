...
#' (EOY - BOY) / BOY using net asset balances directly, while \code{get_rona} uses
...
#' data( dat10k )
#'
#' d <- get_rona( df = dat10k )
#' head( d[ , c( "rona", "rona_w", "rona_z", "rona_p" ) ] )
#'
#' @export
get_rona <- function( df,
                      revenues_less_expenses = "F9_01_EXP_REV_LESS_EXP_CY",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( RONA$rona, na.rm = TRUE ), main = "RONA (raw)" )
    plot( density( RONA$rona_w, na.rm = TRUE ), main = "RONA Winsorized" )
    plot( density( RONA$rona_z, na.rm = TRUE ), main = "RONA Standardized (Z)" )
    plot( density( RONA$rona_p, na.rm = TRUE ), main = "RONA Percentile" )
  }

  return( cbind( df, RONA ) )
}
