...
#' data( dat10k )
#'
#' d <- get_casr( df = dat10k )
#' head( d[ , c( "casr", "casr_w", "casr_z", "casr_p" ) ] )
#'
#' @export
get_casr <- function( df,
                      cash             = "F9_10_ASSET_CASH_EOY",
                      savings          = "F9_10_ASSET_SAVING_EOY",
                      accounts_payable = "F9_10_LIAB_ACC_PAYABLE_EOY",
                      grants_payable   = "F9_10_LIAB_GRANT_PAYABLE_EOY",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( CASR$casr, na.rm = TRUE ), main = "CASR (raw)" )
    plot( density( CASR$casr_w, na.rm = TRUE ), main = "CASR Winsorized" )
    plot( density( CASR$casr_z, na.rm = TRUE ), main = "CASR Standardized (Z)" )
    plot( density( CASR$casr_p, na.rm = TRUE ), main = "CASR Percentile" )
  }

  return( cbind( df, CASR ) )
}
