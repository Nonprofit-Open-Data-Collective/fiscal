...
#'
#' d <- get_ggr( df = dat10k )
#' head( d[ , c( "ggr", "ggr_w", "ggr_z", "ggr_p" ) ] )
#'
#' @export
get_ggr <- function( df,
                     government_grants = "F9_08_REV_CONTR_GOVT_GRANT",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( GGR$ggr, na.rm = TRUE ), main = "GGR (raw)" )
    plot( density( GGR$ggr_w, na.rm = TRUE ), main = "GGR Winsorized" )
    plot( density( GGR$ggr_z, na.rm = TRUE ), main = "GGR Standardized (Z)" )
    plot( density( GGR$ggr_p, na.rm = TRUE ), main = "GGR Percentile" )
  }
