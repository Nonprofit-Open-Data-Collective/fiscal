...
#' data( dat10k )
#'
#' d <- get_moch( df = dat10k )
#' head( d[ , c( "moch", "moch_w", "moch_z", "moch_p" ) ] )
#'
#' @export
get_moch <- function( df,
                      cash                = "F9_10_ASSET_CASH_EOY",
                      savings             = "F9_10_ASSET_SAVING_EOY",
                      pledges_receivable  = "F9_10_ASSET_PLEDGE_NET_EOY",
                      accounts_receivable = "F9_10_ASSET_ACC_NET_EOY",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( MOCH$moch, na.rm = TRUE ), main = "MOCH (raw)" )
    plot( density( MOCH$moch_w, na.rm = TRUE ), main = "MOCH Winsorized" )
    plot( density( MOCH$moch_z, na.rm = TRUE ), main = "MOCH Standardized (Z)" )
    plot( density( MOCH$moch_p, na.rm = TRUE ), main = "MOCH Percentile" )
  }

  return( cbind( df, MOCH ) )
}
