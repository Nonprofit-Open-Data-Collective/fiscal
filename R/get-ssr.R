...
#'
#' d <- get_ssr( df = dat10k )
#' head( d[ , c( "ssr", "ssr_w", "ssr_z", "ssr_p" ) ] )
#'
#' @export
get_ssr <- function( df,
                     program_service_rev = c( "F9_08_REV_PROG_TOT_TOT", "F9_01_REV_PROG_TOT_CY" ),
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( SSR$ssr, na.rm = TRUE ), main = "SSR (raw)" )
    plot( density( SSR$ssr_w, na.rm = TRUE ), main = "SSR Winsorized" )
    plot( density( SSR$ssr_z, na.rm = TRUE ), main = "SSR Standardized (Z)" )
    plot( density( SSR$ssr_p, na.rm = TRUE ), main = "SSR Percentile" )
  }
