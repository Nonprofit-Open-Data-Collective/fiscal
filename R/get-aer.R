...
#'
#' d <- get_aer( df = dat10k )
#' head( d[ , c( "aer", "aer_w", "aer_z", "aer_p" ) ] )
#'
#' @export
get_aer <- function( df,
                     mgmt_expenses  = "F9_09_EXP_TOT_MGMT",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( AER$aer, na.rm = TRUE ), main = "AER (raw)" )
    plot( density( AER$aer_w, na.rm = TRUE ), main = "AER Winsorized" )
    plot( density( AER$aer_z, na.rm = TRUE ), main = "AER Standardized (Z)" )
    plot( density( AER$aer_p, na.rm = TRUE ), main = "AER Percentile" )
  }
