...
#'
#' d <- get_roa( df = dat10k )
#' head( d[ , c( "roa", "roa_w", "roa_z", "roa_p" ) ] )
#'
#' @export
get_roa <- function( df,
                     revenues_less_expenses = "F9_01_EXP_REV_LESS_EXP_CY",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( ROA$roa, na.rm = TRUE ), main = "ROA (raw)" )
    plot( density( ROA$roa_w, na.rm = TRUE ), main = "ROA Winsorized" )
    plot( density( ROA$roa_z, na.rm = TRUE ), main = "ROA Standardized (Z)" )
    plot( density( ROA$roa_p, na.rm = TRUE ), main = "ROA Percentile" )
  }
