...
#' data( dat10k )
#'
#' d <- get_luna( df = dat10k )
#' head( d[ , c( "luna", "luna_w", "luna_z", "luna_p" ) ] )
#'
#' @export
get_luna <- function( df,
                      unrestricted_net_assets = "F9_10_NAFB_UNRESTRICT_EOY",
                      net_fixed_assets        = "F9_10_ASSET_LAND_BLDG_NET_EOY",
                      mortgages_payable       = "F9_10_LIAB_MTG_NOTE_EOY",
                      total_expenses          = "F9_09_EXP_TOT_TOT",
                     ...
                     summarize = FALSE )
{
  ...
  if ( summarize ) {
    ...
    plot( density( LUNA$luna, na.rm = TRUE ), main = "LUNA (raw)" )
    plot( density( LUNA$luna_w, na.rm = TRUE ), main = "LUNA Winsorized" )
    plot( density( LUNA$luna_z, na.rm = TRUE ), main = "LUNA Standardized (Z)" )
    plot( density( LUNA$luna_p, na.rm = TRUE ), main = "LUNA Percentile" )
  }

  return( cbind( df, LUNA ) )
}
